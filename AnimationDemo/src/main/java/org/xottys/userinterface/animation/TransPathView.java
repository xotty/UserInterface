/**
 * 本例演示了用自定义路径方式（类似矢量图）来构造视图及其动画，要点如下：
 * 1）构造自定义Path数据类：SVGAction
 * 2）解析自定义Path数据，将其转换为SVGAction对象：buildActions()
 * 3）用解析后的数据绘制路径：actionPath()
 * 4）用ValueAnimator方式设置动画，它通过宿主程序启动：startTrans()
 * 5）onAnimationUpdate中不断根据动画进程不断调用actionPath来绘制当前路径，并旋转视图
 * 6）onDraw中用canvas.drawPath(path, paint)绘图
 *
 * <p>
...
public class TransPathView extends View implements ValueAnimator.AnimatorUpdateListener,
    ...
    private void refresh() {
        path.reset();
        //根据动画过程（0～1）计算当前需要画的线，然后勾画当前的数字
        for (SVGAction a : actions) {
